...
convertToSphericalCoords(NUI_SKELETON_DATA *users, unsigned useridx) {
  ...
  leftSholderNite.x /= leftSholderNite.w;
  leftSholderNite.y /= leftSholderNite.w;
  leftSholderNite.z /= leftSholderNite.w;

  rightSholderNite.x /= rightSholderNite.w;
  rightSholderNite.y /= rightSholderNite.w;
  rightSholderNite.z /= rightSholderNite.w;

  rightHipNite.x /= rightHipNite.w;
  rightHipNite.y /= rightHipNite.w;
  rightHipNite.z /= rightHipNite.w;

  rightHandNite.x /= rightHandNite.w;
  rightHandNite.y /= rightHandNite.w;
  rightHandNite.z /= rightHandNite.w;
  ...
  float alpha = glm::acos(raTDot / (glm::length(tcNorm) *
                          glm::length(rightArm)));
  ...
  return glm::vec3{glm::length(rightArm), alpha, phi};
}
