...
public:
  ...
  static LRESULT CALLBACK MessageRouter(HWND hWnd, UINT message, WPARAM wParam, 
                                        LPARAM lParam);
  ...
  /// <param name="lParam">additional message data</param>
  /// <returns>result of message processing</returns>
  LRESULT CALLBACK DlgProc(HWND hWnd, UINT message, WPARAM wParam, 
                           LPARAM lParam);
  ...
  int Run(HINSTANCE hInstance, int nCmdShow);
                      ...
                      ///////////////////////
  void drawProbGraph(const std::vector<float> &pVec);

  bool cutBoundingBox(D2D1_POINT_2F leftHandPos, Vector4 camPos);

private:
  std::future<std::vector<float>> m_classifyResult;
  sf::TcpSocket sock;
  std::future<std::vector<float>> _classifyFuture;

  cv::Mat imgToClassify;
  std::vector<int> im50x50Cut;
  std::vector<float> predVec;

  std::vector<float> imSeg, imBinSeg;
  ...
  void DrawSkeleton(const NUI_SKELETON_DATA & skel, int windowWidth, 
                    int windowHeight);
...
std::vector<float> ClassifyImgNet(sf::TcpSocket &sock,
